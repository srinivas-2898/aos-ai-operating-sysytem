...
def generate_html_screen(project_name: str, app_type: str, screen_name: str, features: list, description: str, platform: str = "mobile") -> str:
    ...
    system_prompt = (
        "You are an expert front-end developer and UI/UX designer. Generate complete, single-file HTML & CSS code for a premium UI screen.\n\n"
        "Requirements:\n"
        f"- Target Screen Name: {screen_name}\n"
        f"- Layout Style: {'Mobile layout (390px width by 844px height)' if is_mobile else 'Desktop layout (full 16:9 widescreen)'}\n"
        f"- App Description & Prompt: {description}\n"
        f"- Core Features: {', '.join(features) if features else 'Standard features'}\n\n"
        "CRITICAL Design Instructions:\n"
        "- Return ONLY valid HTML. Do NOT wrap in markdown (no ```html). No explanations.\n"
        "- Keep CSS concise: use shorthand properties, avoid excessive comments. The COMPLETE HTML document including </html> MUST fit within 7000 characters.\n"
        "- Customize all text, labels, buttons to match the app description. Prioritize the user's prompt for branding.\n"
        "- For images/logos/avatars, use: src=\"ai-image:<description of desired graphic>\". Example: <img src=\"ai-image:modern app logo, minimal vector\" />\n"
        "- Use modern styling: gradients, rounded corners, shadows, clean typography (Google Fonts link to Inter or Poppins). No Lorem Ipsum.\n"
        "- IMPORTANT: You MUST output the COMPLETE HTML document from <!DOCTYPE html> all the way through to the closing </html> tag. Never truncate."
    )
